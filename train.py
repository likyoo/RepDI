...
# python train.py E:/dataset/苹果叶部病理图像_飞桨/original/original/ --model efficientnet_b0 --lr 0.1 --warmup-epochs 5 --epochs 10 --weight-decay 1e-4 --sched cosine -b 8 
""" 
From timm.
"""
import argparse
...
def main():
    ...
    if args.model == 'shufflenet_v2_x2_0':
        ...
        if args.model == "mobileone":
            model = custom_models.mobileone(variant='s3', num_classes=args.num_classes)
        elif args.model == "pdcnet":
            model = custom_models.pdcnet(variant='s1', num_classes=args.num_classes)
        else:
    ...
    # dataset_train = Paddle_Apple_Dataset(
    #     root=args.data_dir,
    #     split_file=osp.join(args.data_dir, 'train.list'),
    #     train=True
    # )
    # dataset_eval = Paddle_Apple_Dataset(
    #     root=args.data_dir,
    #     split_file=osp.join(args.data_dir, 'test.list'),
    #     train=True
    # )
    dataset_train = SAAS_Pdc_Apple_Dataset(
        root=args.data_dir,
        split_file=osp.join(args.data_dir, 'train_saas_pdc_apple_leaf_20220808.list'),
        train=True
    )
    dataset_eval = SAAS_Pdc_Apple_Dataset(
        root=args.data_dir,
        split_file=osp.join(args.data_dir, 'test_saas_pdc_apple_leaf_20220808.list'),
        train=True
    )
    ...
    loader_train = create_loader(
        ...
        resize_first_with_crop_pct=0, # 在数据增强之前先进行一下resize，防止输入图像太大
    )
    ...
    try:
        ...
        # Get Time
        infer(model, loader_eval, args, amp_autocast=amp_autocast)
...
def train_one_epoch(
    ...
    model.train()
    try:
        if model.__name__ in ['mobileone', 'pdcnet']:
            model.inference_mode = False
    except:
        print('Warning: It is not reparameterize model now')
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    try:
        if model.__name__ in ['mobileone', 'pdcnet']:
            ...
            print(f'Warning: The {model.__name__} is not support for `reparameterize_model`.')
    except:
        print('Warning: It is not reparameterize model now')
...
def infer(model, loader, args, amp_autocast=suppress):
