...
class ECABlock(nn.Module):
    ...
    def __init__(self, in_channels, k_size=3):
        ...
        self.conv = nn.Conv1d(1, 1, kernel_size=k_size, padding=(k_size - 1) // 2 + 1, bias=False, dilation=2)
...
class PDAM(nn.Module):
    # Parallel Dilation Attention Module
    def __init__(self,
                 in_channels: int,
                 k_size: int = 3) -> None:
        """ Construct a Squeeze and Excite Module.
        ...
        # self.conv = nn.Conv1d(1, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False, dilation=1)
        dilation = 2
        self.channel_conv = nn.Conv1d(1, 1, kernel_size=k_size, padding=(k_size - 1) // 2 + 1, bias=False, dilation=dilation)
        self.sigmoid = nn.Sigmoid()

        self.spatial_conv = nn.Conv2d(1, 1, kernel_size=k_size, bias=False, padding=(k_size - 1) // 2 + 1, dilation=dilation)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """ Apply forward pass. """
        x1 = self.avg_pool(x)
        x1 = self.channel_conv(x1.squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1)
        x1 = x1.expand_as(x)
...
class PDCBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if use_se:
            ...
            # self.se = ECABlock(out_channels)
            self.se = PDAM(out_channels)
            # self.se = nn.Identity()

            # self.se = CoordAtt(out_channels)
            # self.se = sa_layer(out_channels)
        else:
            self.se = nn.Identity()
...
class PDCNet(nn.Module):
    ...
    def __init__(self,
        ...
        # Build stages
        self.stage0 = PDCBlock(in_channels=3, out_channels=self.in_planes,
        ...
        # self.shuffle_head = InvertedResidual(int(512 * width_multipliers[3]), int(512 * width_multipliers[3]), 1, 1)

        # self.last_conv = nn.Conv2d(
        #     in_channels=int(512 * width_multipliers[3]),
        #     out_channels=int(512 * width_multipliers[3]),
        #     kernel_size=1,
        #     stride=1,
        #     padding=0,
        #     bias=False)
        ...
        # self.fc = nn.Linear(int(512 * width_multipliers[3]), num_classes)
    ...
    def _make_stage(self,
        ...
        for ix, stride in enumerate(strides):
            ...
            # Depthwise conv
            blocks.append(PDCBlock(in_channels=self.in_planes,
...
def pdcnet(num_classes: int = 1000, inference_mode: bool = False,
    ...
    return PDCNet(num_classes=num_classes, inference_mode=inference_mode,
                     **variant_params)
